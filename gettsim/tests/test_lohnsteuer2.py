...
def input_data():
    ...
    lst_data = pd.read_csv(ROOT_DIR / "tests" / "test_data" / "lohnsteuer_testfälle_bmf_2022.csv", header=5)
    ...
    var_names = {
        ...
        "krv": "wohnort_ost",
        "re4": "lst_wage", # * 100
        "lzz": "period_of_obtained_wage", # not yet implemented
        "lstlzz": "lohn_st",
    ...
    # test_data = test_data.set_index(["tu_id", "hh_id", "p_id"]).sort_index()

    # Take into account period of obtained wage (Lohnzahlungszeitraum, LZZ)
    test_data.loc[test_data["period_of_obtained_wage"] == 4, "bruttolohn_m"] = test_data["lst_wage"] * 7
