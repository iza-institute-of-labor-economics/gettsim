...
OUT_COLS = ["wohngeld_vor_vermög_check_m_hh"]
...
def input_data():
    return pd.read_csv(ROOT_DIR / "tests" / "test_data" / "wohngeld.csv")
...
def test_wohngeld(input_data, year, column):
    ...
    assert_series_equal(result[column].astype(float), year_data[column], atol=0)
