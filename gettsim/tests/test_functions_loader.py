...
def test_load_modules():
    assert _load_functions("gettsim.social_insurance_payments.krankenv")


def test_load_path():
    assert _load_functions(ROOT_DIR / "social_insurance_payments" / "krankenv.py")
...
def test_special_attribute_module_is_set_for_internal_functions():
    out = _load_functions("gettsim.social_insurance_payments.eink_grenzen")
    function = out[list(out)[0]]

    assert function.__module__ == "gettsim.social_insurance_payments.eink_grenzen"
