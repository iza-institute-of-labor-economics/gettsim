...
def test_length_column_names_user_facing():
    ...
    check_length(
        column_names=user_facing_names, limit=GEP_01_CHARACTER_LIMIT_USER_FACING
    )


def test_length_column_names_internal(all_column_names):
    internal_names = [c for c in all_column_names if c not in DEFAULT_TARGETS]
    check_length(column_names=internal_names, limit=GEP_01_CHARACTER_LIMIT_INTERNAL)
