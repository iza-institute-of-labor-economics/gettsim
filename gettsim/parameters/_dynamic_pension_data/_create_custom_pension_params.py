...
def lohnkomponente(ges_rente_params, year):
    ...
    return ges_rente_params[f"durchschnittslohn_{year - 1}"] / (
        ...
        * (
            ...
            / (
                ges_rente_params[f"beitragspflichtiger_durchschnittslohn_{year - 2}"]
                / ges_rente_params[f"beitragspflichtiger_durchschnittslohn_{year - 3}"]
            )
        )
    )
...
def rentnerquotienten(ges_rente_params, soz_vers_beitr_params, year):
    ...
    """
    return (
        ges_rente_params[f"ges_rentenvolumen_{year}"]
