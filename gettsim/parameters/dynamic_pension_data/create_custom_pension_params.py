...
def brechne_rentenwert_aus_daten(ges_renten_vers_params, year):
    ...
    lohn = lohnkomponente(ges_renten_vers_params, year)
    # Second riesterfaktor
    riester = riesterfaktor(ges_renten_vers_params, year)
    # Nachhaltigskeitsfaktor
    nachhfaktor = nachhaltigkeitsfaktor(ges_renten_vers_params, year)

    # Rentenwert must not be lower than in the previous year.
    renten_factor = lohn * riester * nachhfaktor
    rentenwert = ges_renten_vers_params[f"rentenwert_{year - 1}"] * min(
        1, renten_factor
    )
    return rentenwert


def lohnkomponente(ges_renten_vers_params, year):
    """Returns the lohnkomponente for each year. It deppends on the average wages of
    the previous years. For details see
    https://de.wikipedia.org/wiki/Rentenanpassungsformel
    """
    return ges_renten_vers_params[f"durchschnittslohn_{year - 1}"] / (
        ges_renten_vers_params[f"durchschnittslohn_{year - 2}"]
        * (
            (
                ges_renten_vers_params[f"durchschnittslohn_{year - 2}"]
                / ges_renten_vers_params[f"durchschnittslohn_{year - 3}"]
            )
            / (
...
def riesterfaktor(ges_renten_vers_params, soz_vers_beitr_params, year):
    ...
    return (
        100
        - ges_renten_vers_params[f"altersvorsogeanteil_{year - 1}"]
        - soz_vers_beitr_params[f"rvbeitrag_{year - 1}"]
    ) / (
        100
        - ges_renten_vers_params[f"altersvorsogeanteil_{year - 2}"]
        - soz_vers_beitr_params[f"rentenv_{year - 2}"]
    )


def nachhaltigkeitsfaktor(ges_renten_vers_params, year):
    ...
    https://de.wikipedia.org/wiki/Rentenanpassungsformel
    """
    rq_last_year = rentnerquotienten(ges_renten_vers_params, year - 1)
    rq_two_years_before = rentnerquotienten(ges_renten_vers_params, year - 2)
...
def rentnerquotienten(ges_renten_vers_params, soz_vers_beitr_params, year):
    """The rentnerquotient is the relationship between pension insurance receivers and
    contributes. For details see
    https://de.wikipedia.org/wiki/Rentenanpassungsformel
    """
    return (
        ges_renten_vers_params[f"rentenvolumen_{year}"]
        / ges_renten_vers_params[f"eckrente_{year}"]
    ) / (
        ges_renten_vers_params[f"beitragsvolumen_{year}"]
        / (
