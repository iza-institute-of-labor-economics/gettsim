...
def favorability_check(tax_unit, e_st_abzuege_params):
    """ 'Higher-Yield Test'
    ...
    max_inc = get_max_inc(tax_unit, e_st_abzuege_params)
    # relevant incometax
    ...
    if (not ("nokfb" in max_inc)) | (e_st_abzuege_params["year"] <= 1996):
...
def get_max_inc(tax_unit, e_st_abzuege_params):
    ...
    inc_list = []
    for i, inc in enumerate(e_st_abzuege_params["zve_list"]):
