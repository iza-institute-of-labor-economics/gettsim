...
def pensions(person, soz_vers_beitr_data, ges_renten_vers_data):
    """
    ...
    person = update_earnings_points(
        person, soz_vers_beitr_data, ges_renten_vers_data, year
    )
    # ZF: Zugangsfaktor.
...
def update_earnings_points(person, soz_vers_beitr_data, ges_renten_vers_data, year):
    ...
    out = _ep_for_earnings(person, soz_vers_beitr_data, ges_renten_vers_data, year)
    out += _ep_for_care_periods(person, soz_vers_beitr_data, ges_renten_vers_data)
...
def _ep_for_earnings(person, soz_vers_beitr_data, ges_renten_vers_data, year):
    """Return earning points for the wages earned in the last year."""
    westost = "o" if person["east"] else "w"
    return (
        np.minimum(person["m_wage"], soz_vers_beitr_data["rvmaxek" + westost])
        / ges_renten_vers_data[f"meanwages_{year}"]
    )


def _ep_for_care_periods(df, soz_vers_beitr_data, ges_renten_vers_data):
...
def _rentenwert_until_2017(ges_renten_vers_data, year):
    """For the years until 2017, we use a exogenous value for the rentenwert."""
    return ges_renten_vers_data[f"rentenwert_ext_{year}"]


def _rentenwert_from_2018(ges_renten_vers_data, year):
    ...
    lohnkomponente = _lohnkomponente(ges_renten_vers_data, year)
    # Second riesterfaktor
    riesterfaktor = _riesterfactor(ges_renten_vers_data, year)
    # Nachhaltigskeitsfaktor
    nachhfaktor = _nachhaltigkeitsfaktor(ges_renten_vers_data, year)

    # Rentenwert must not be lower than in the previous year.
    renten_factor = lohnkomponente * riesterfaktor * nachhfaktor
    rentenwert = ges_renten_vers_data[f"rentenwert_ext_{year - 1}"] * min(
        1, renten_factor
    )
    return rentenwert


def _lohnkomponente(ges_renten_vers_data, year):
    """Returns the lohnkomponente for each year. It deppends on the average wages of
    the previous years. For details see
    https://de.wikipedia.org/wiki/Rentenanpassungsformel
    """
    return ges_renten_vers_data[f"meanwages_{year - 1}"] / (
        ges_renten_vers_data[f"meanwages_{year - 2}"]
        * (
            (
                ges_renten_vers_data[f"meanwages_{year - 2}"]
                / ges_renten_vers_data[f"meanwages_{year - 3}"]
            )
            / (
                ges_renten_vers_data[f"meanwages_sub_{year - 2}"]
                / ges_renten_vers_data[f"meanwages_sub_{year - 3}"]
            )
        )
    )


def _riesterfactor(ges_renten_vers_data, year):
    ...
    return (
        100
        - ges_renten_vers_data[f"ava_{year - 1}"]
        - ges_renten_vers_data[f"rvbeitrag_{year - 1}"]
    ) / (
        100
        - ges_renten_vers_data[f"ava_{year - 2}"]
        - ges_renten_vers_data[f"rvbeitrag_{year - 2}"]
    )


def _nachhaltigkeitsfaktor(ges_renten_vers_data, year):
    ...
    https://de.wikipedia.org/wiki/Rentenanpassungsformel
    """
    rq_last_year = _rentnerquotienten(ges_renten_vers_data, year - 1)
    rq_two_years_before = _rentnerquotienten(ges_renten_vers_data, year - 2)
    # There is an additional 'Rentenartfaktor', equal to 1 for old-age pensions.
    return 1 + (
        (1 - (rq_last_year / rq_two_years_before))
        * ges_renten_vers_data[f"alpha_{year}"]
    )


def _rentnerquotienten(ges_renten_vers_data, year):
    """The rentnerquotient is the relationship between pension insurance receivers and
    contributes. For details see
    https://de.wikipedia.org/wiki/Rentenanpassungsformel
    """
    return (
        ges_renten_vers_data[f"rentenvol_{year}"]
        / ges_renten_vers_data[f"eckrente_{year}"]
    ) / (
        ges_renten_vers_data[f"beitragsvol_{year}"]
        / (
            ges_renten_vers_data[f"rvbeitrag_{year}"]
            / 100
            * ges_renten_vers_data[f"eckrente_{year}"]
        )
    )
