...
def test_all_default_targets_among_function_names(function_names):
    check = [
        ...
        if (c not in function_names) and (remove_group_suffix(c) not in function_names)
    ]
...
def test_length_column_names_other_functions(function_names):
    ...
    other_function_names = [
        c for c in function_names if c not in DEFAULT_TARGETS and not c.startswith("_")
    ]
