    ...
    data = pd.DataFrame(
        {
            "p_id": [1, 2, 3],
            "vg_id": [1, 1, 2],
            "arbeitsl_geld_2_m_vg": [100, 200, 300],
        }
    )
...
def test_user_provided_aggregation_specs():
    ...
    out = compute_taxes_and_transfers(
        ...
        targets="arbeitsl_geld_2_m_vg",
    )

    numpy.testing.assert_array_almost_equal(out["arbeitsl_geld_2_m_vg"], expected_res)
...
def test_user_provided_aggregation_specs_function():
    ...
    out = compute_taxes_and_transfers(
        ...
        aggregation_specs=aggregation_specs,
        targets="arbeitsl_geld_2_double_m_vg",
    )

    numpy.testing.assert_array_almost_equal(
        out["arbeitsl_geld_2_double_m_vg"], expected_res
    )
