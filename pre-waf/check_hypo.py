...
def check_hypo(settings):
    ...
    try:
        stata = pd.read_stata(
            settings["DATA_PATH"] + "hypo/stata_check_" + taxyear + ".dta",
    ...
    pyth = pd.read_json(settings["DATA_PATH"] + "hypo/python_checkRS" + taxyear + ".json")
    ...
    # merge them
    df = pd.merge(pyth, stata, how="inner", on=["typ_bud", "y_wage"], suffixes=["_pyth", "_stata"])

    df = df.sort_values(by=["typ_bud", "y_wage"])
    # check difference in dpi
    for var in ["dpi", "kindergeld_hh", "tax", "svbeit", "m_alg2", "kiz", "wohngeld"]:
        ...
        for typ in df["typ_bud"].unique():
            ...
            ax.plot(grp["m_wage_pyth"], grp[var + "_pyth"], "k-", label="Python")
            ax.plot(grp["m_wage_pyth"], grp[var + "_stata"], "k:", label="Stata")
            ax.set_title(str(types[typ]), size=14)
            ax.set_ylabel(var, size=14)
            ax.set_xlabel("Gross Wage")
            ax.legend(loc="best")
            plt.savefig(settings["GRAPH_PATH"] + "hypo/check_" + var + "_" + str(typ) + ".png")
