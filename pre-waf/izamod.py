...
def run_izamod(settings):
    ...
    if settings["load_data"] == 1:
        rawdata = loaddata(
            settings["SOEP_PATH"], settings["DATA_PATH"] + "SOEP/", settings["minyear"]
        )

        print(
            "Save " + str(rawdata.shape) + " Data to: " + settings["DATA_PATH"] + "SOEP/soep_long"
        )
    ...
    if settings["prepare_data"] == 1:
        ...
        summaries.to_excel(
            pd.ExcelWriter(settings["DATA_PATH"] + "SOEP/sum_data_out.xlsx"), sheet_name="py_out"
        )
        ...
        if settings["show_descr"] == 1:
            ...
            descr.to_excel(
                pd.ExcelWriter(settings["DATA_PATH"] + "SOEP/data_descr.xlsx"), sheet_name="descr"
            )
    ...
    if (settings["taxtrans"] == 1) or (settings["run_hypo"] == 1):
        ...
        tb_pens = pd.read_excel(
            settings["MAIN_PATH"] + "/data/params/pensions.xlsx", index_col="var"
        ).transpose()
    ...
    if settings["taxtrans"] == 1:
        for ref in settings["Reforms"]:
            ...
            if ref != "UBI":
                tt_out = tax_transfer(df, datayear, settings["taxyear"], tb, tb_pens, mw, False)
            else:
                tb_ubi = ubi_settings(tb)
                tt_out = tax_transfer_ubi(
                    df, datayear, settings["taxyear"], tb_ubi, tb_pens, mw, False
                )
...
print("-" * 80)
print("Total time used: " + time.strftime("%H:%M:%S", time.gmtime(end - start)) + " Hours.")
print("-" * 80)
