...
def tax_transfer(df, datayear, taxyear, tb, tb_pens=[], mw=[], hyporun=False):
    ...
    df = df.join(other=wg(df, tb, taxyear), how="inner")
...
def zve(df, tb, yr, hyporun, ref=""):
    ...
    zve["kifreib"] = 0
    zve["kifreib"] = 0.5 * tb["kifreib"] * df["child_num_tu"] * ~df["child"]
    # Taxable income (zve)
...
def favorability_check(df, tb, yr):
    ...
    fc = fc.join(fc.groupby(["hid"])["kindergeld"].sum(), on=["hid"], how="left", rsuffix="_hh")
...
def wg(df, tb, yr):
    ...
    wg["wg_otherinc"] = wg["m_alg1_tu_k"] + wg["m_transfers_tu_k"] + (wg["pens_steuer_tu_k"])
...
def alg2(df, tb, yr):
    ...
    for var in ["ar_alg2_ek", "alg2_grossek"]:
        alg2 = alg2.join(alg2.groupby(["hid"])[var].sum(), on=["hid"], how="left", rsuffix="_hh")
...
def kiz(df, tb, yr):
    ...
    kiz.loc[kiz["kiz_incrange"], "kiz"] = np.maximum(
        0, (tb["a2kiz"] * df["child_num_tu"]) - kiz["kiz_ek_anr"]
    )
