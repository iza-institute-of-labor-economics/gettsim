...
def call_heckman(df, plot=False):
    ...
    df["ln_wage"] = np.select(
        [df["h_wage"] > 0, df["h_wage"] == 0], [np.log(df["h_wage"]), np.nan]
    )
    ...
    for yr in df["syear"].unique():
        ...
        for f in [0, 1]:
            ...
            p1p99 = df["h_wage"][
                (df["h_wage"] > 0) & (df["syear"] == yr) & (df["female"] == f)
            ].describe(percentiles=[0.01, 0.99])[["1%", "99%"]]
            # define subsample
            sub = (
                (df["syear"] == yr)
                & (df["female"] == f)
    ...
    if plot:
        for yr in df["syear"].unique():
            for f in [0, 1]:
                ...
                ax_heck = df["heck_wage"][
                    (df["syear"] == yr) & (df["female"] == f) & (df["heck_wage"].between(5, 100))
                ].plot.kde(label="Heckman Wage")
                ax_wage = df["h_wage"][
                    (df["syear"] == yr) & (df["female"] == f) & (df["h_wage"].between(5, 100))
                ].plot.kde(label="Observed Wage")
                ax_heck.set_xlim(0, 100)
                ax_heck.legend(loc=0, ncol=1, fontsize=14, frameon=True)

                plt.savefig(
                    get_settings()["GRAPH_PATH"]
                    + "wageplots/wageplots_y"
                    + str(yr)
                    + "_f"
                    + str(f)
                    + ".png"
...
def heckman(wage_y, wage_X, wage_X2, select_y, select_X):
    ...
    probit_model = sm.Probit(np.asarray(select_y), np.asarray(select_X))
...
def preparedata(df):
    ...
    df = df.join(
        df.groupby(["syear", "hid"])["pnr"].max(), on=["syear", "hid"], how="left", rsuffix="_max"
    )
    ...
    df = df.join(
        df.groupby(["syear", "hid"])["age_mis"].sum(),
        on=["syear", "hid"],
        how="left",
        rsuffix="_sum",
    )
    ...
    df["ineducation"] = (
        (((df["pgstib"] > 100) & (df["pgstib"] < 200)) | (df["pgstib"] == 11))
        | (df["ks_ats_r"] == 1)
        | (df["age"] < 18)
    )
    ...
    df["child"] = ((df["stell"] == 3) & (df["age"] < 16)) | (
        (df["age"] <= 16) | ((df["age"] < 25) & df["ineducation"])
    )
    # other members
    ...
    df = df.join(
        df.groupby(["syear", "hid"])["young_couple_unit"].sum(),
        on=["syear", "hid"],
        how="left",
        rsuffix="_sum",
    )
    # if there is only one partner, it's a single
    df.loc[(df["young_couple_unit_sum"] == 1) & (~df["main_unit"]), "young_single_unit"] = True
    ...
    df = df.join(
        df.groupby(["syear", "hid"])["old_dummy"].sum(),
        on=["syear", "hid"],
        how="left",
        rsuffix="_sum",
    )
    ...
    df.loc[(~df["main_unit"]) & (df["single_unit"]) & (~df["couple_unit"]), "new_unit"] = True
    df.loc[
        (~df["main_unit"]) & (df["single_unit"]) & (~df["couple_unit"]) & (df["child"]), "new_unit"
    ] = False
    df.loc[(~df["main_unit"]) & (~df["single_unit"]) & (df["couple_unit"]), "new_unit"] = True
    # Set 'new_unit' to zero for the second partner of a couple unit.
    df = df.sort_values(
        by=["syear", "hid", "couple_unit", "single_unit"], ascending=[True, True, False, False]
    )
    df["n"] = df.groupby(["syear", "hid"])["counter"].cumsum()
    df.loc[
        (~df["main_unit"]) & (~df["single_unit"]) & (df["couple_unit"]) & (df["n"] > 1), "new_unit"
    ] = False
    # Produce new unit id
    df = df.sort_values(
        by=["syear", "hid", "head", "main_unit", "single_unit", "couple_unit", "new_unit", "pnr"],
    ...
    # size of tax unit
    df = df.join(
        df.groupby(["syear", "tu_id"])["counter"].sum(),
        on=["syear", "tu_id"],
        how="left",
        rsuffix="_tu",
    )
    ...
    ch_ages = {
        "0_1": pd.eval("df.child and df.age <= 1"),
    ...
    for id in ["hid", "tu_id"]:
        for c in ch_ages:
            ...
            if id == "tu_id":
                c = c + "_tu"
            df = df.join(
                df.groupby(["syear", id])["temp"].sum(), on=["syear", id], how="left", rsuffix=c
            )
            df = df.rename(columns={"temp" + c: "child" + c})
            df["child" + c] = df["child" + c].astype(int)

    df = df.join(
        df.groupby(["syear", "tu_id"])["child"].sum(),
        on=["syear", "tu_id"],
        how="left",
        rsuffix="_num_tu",
    )
    ...
    df = df.join(
        df.groupby(["syear", "hid"])["counter"].first(), on=["syear", "hid"], rsuffix="_first"
    )
    ...
    df = df.join(
        df.groupby(["syear", "hid"])["child"].sum(),
        on=["syear", "hid"],
        how="left",
        rsuffix="_num",
    )
    ...
    print(
        "Deleting incorrect Tax Units..."
        + str(df["counter"][df["adult_num_tu"] > 2].sum())
        + " persons."
    )
    ...
    sel_quali = [
        df["pgcasmin"].between(0, 2),
        df["pgcasmin"].between(3, 6),
        df["pgcasmin"].between(7, 9),
    ]
    sel_isced = [
        df["pgisced11"].between(0, 2),
    ...
    df.loc[
        (df["pglabgro"] > 0)
        & ((pd.isna(df["months"])) | (df["months"] == 0) | (df["months"] <= 0)),
        "months",
    ] = 12
    ...
    df["pensions_priv"] = df[
        ["ivbl1", "ivbl2", "icom1", "icom2", "iprv1", "iprv2", "ison1", "ison2", "irie1", "irie2"]
    ].sum(axis=1)
    df["pensions"] = df[["pensions_pub", "pensions_priv"]].sum(axis=1)
    # correct implausible values for months of pensions
    df.loc[
        ((df["months_pen"] == 0) | (pd.isna(["months_pen"]))) & (df["pensions"] > 0), "months_pen"
    ] = 12
    df.loc[(pd.isna(df["months_pen"])) & (df["pensions"] == 0), "months_pen"] = 0

    df["months_pen"] = df["months_pen"].fillna(0)
    df["m_pensions"] = np.select(
        [df["months_pen"] > 0, df["months_pen"] == 0], [df["pensions"] / df["months_pen"], 0]
    )
    ...
    df["m_trans2"] = (
        df["head_tu"]
        * (df[["nursh", "sphlp", "hsup", "ssold", "chsub"]].sum(axis=1) / 12)
        * df["hh_korr"]
    )
    ...
    df["versbez"] = np.select(
        [df["months_pen"] > 0, df["months_pen"] == 0],
        [df[["iciv1", "ivbl1", "iciv2", "ivbl2"]].sum(axis=1) / df["months_pen"], 0],
    )

    # Dummy for welfare recipient
    df["welfare"] = np.select(
        [df["syear"] <= 2004, df["syear"] >= 2005],
        [
            (df["D_sozh_current"] == 1) | (df["D_wg_current"] == 1) | (df["D_alg_current"] == 1),
            (df["D_sozh_current"] == 1)
            | (df["D_wg_current"] == 1)
            | (df["D_kiz_current"] == 1)
            | (df["D_alg2_current"] == 1),
    ...
    df["w_hours"] = np.select(
        [
            (df["pguebstd"] == 0) | (df["plb0605"] == 0),
    ...
    df["m_wage"] = 0
    df["m_self"] = 0
    df["othwage_ly"] = df[["i13ly", "i14ly", "ixmas", "iholy", "igray", "iothy", "itray"]].sum(
        axis=1
    )
    ...
    df.loc[
        (~df["selfemployed"]) & (~df["pensioner"]) & (~df["military"]) & (~df["parentalleave"]),
        "m_wage",
    ] = df["pglabgro"] + np.maximum(0, df["othwage_ly"] / df["months"])
    df.loc[
        (df["selfemployed"]) & (~df["pensioner"]) & (~df["military"]) & (~df["parentalleave"]),
        "m_self",
    ] = df["pglabgro"] + np.maximum(0, df["othwage_ly"] / df["months"])
    ...
    # Hourly Wage
    df["h_wage"] = np.select(
        [df["w_hours"] == 0, df["w_hours"] > 0], [0, df["m_wage"] / (df["w_hours"] * 4.34)]
    )
    # df.loc[df['h_wage'] > 0, 'h_wage'] = np.minimum(3, df['h_wage'])
    df["worker"] = ~df["child"] * (df["m_wage"] > 0)
    df = df.join(
        df.groupby(["syear", "tu_id"])["worker"].sum(),
        on=["syear", "tu_id"],
        how="left",
        rsuffix="_sum",
    )
    ...
    df = df.join(
        df.groupby(["syear", "bula", "hhsize"])["hgsize"].mean(),
        on=["syear", "bula", "hhsize"],
        how="left",
        rsuffix="_mean",
    )
    df["wohnfl"] = np.select(
        [pd.isna(df["hgsize"]), ~pd.isna(df["hgsize"])], [df["hgsize_mean"], df["hgsize"]]
    )
    # Construction Year. sometimes important for housing benefit
    df.loc[(~(pd.isna(df["hgcnstyrmax"])) & (~pd.isna(df["hgcnstyrmin"]))), "baujahr"] = (
        df["hgcnstyrmax"] + df["hgcnstyrmin"]
    ) / 2
    sel_cnstyr = [
        df["baujahr"].between(1800, 1965),
        (df["baujahr"].between(1966, 2000)) | (pd.isna(df["baujahr"])),
    ...
    for var in ["rent", "heat"]:
        ...
        est = pd.concat(
            [
                df[["hg" + var, "eigentum"]],
                pd.get_dummies(df["cnstyr"], prefix="cns", drop_first=True),
                pd.get_dummies(df["bula"], prefix="bl", drop_first=True),
                pd.get_dummies(df["syear"], prefix="y", drop_first=True),
            ],
            axis=1,
        )
        sub = est[(~pd.isna(est["hg" + var])) & (~est["eigentum"])]

        imp_rent = sm.OLS(
            sub["hg" + var], sub.drop(columns=["hg" + var, "eigentum"]).astype(float)
        ).fit()
        # Predict Miete/Heizkosten for all missings + non-owners
        est.loc[(est["hg" + var].isna()) & (~est["eigentum"]), "hg" + var] = imp_rent.predict(
            est.drop(columns=["hg" + var, "eigentum"])
        )
        if var == "rent":
    ...
    df["kapdienst"] = (
        np.select([df["eigentum"], ~df["eigentum"]], [df["zinszahl"] + 36 / 12 * df["wohnfl"], 0])
        * df["hh_korr"]
    )
