...
def create_hypo_data(data_path,settings):
    ...
    # Erster Mann ist immer Head
    df.loc[(df['female'] == True) & df['child'] == False, 'head'] = False
    df.loc[(df['female'] == True) & df['child'] == False, 'head_tu'] = False
    ...
    tuvars = ['child3_6','child7_11','child7_13','child7_16','child12_15','child14_24',
              'child2','child11','child15','child18','child','adult']
    ...
    # Teile das Jahreseinkommen auf für Paare...es gibt nur alleinverdiener
    df['y_wage_ind'] = df['y_wage']
    df.loc[df['female'] == True, 'y_wage_ind'] = 0
    ...
    taxout_hypo = taxtransfer(settings['DATA_PATH']+'SOEP/taxben_input_',settings,'RS2018',True)
    ...
    out_vars = ['typ_bud','female','age','head','child','y_wage','m_wage','w_hours','dpi',
                'wohngeld','kiz','kindergeld','svbeit','tax_income','incometax']
    for typ in [11,22,24,31,32]:
        df[df['typ_bud']==typ].to_excel(hypo_writer,sheet_name='typ_'+str(typ),columns=out_vars,na_rep='NaN',freeze_panes=(1,0))

    ## graph it
    # data set with heads only
    h = df[df['head']]
    h = h.sort_values(by = ['typ_bud','y_wage'])
    h['emtr'] = 1 - (h['dpi'] - h['dpi'].shift(1))/ (h['m_wage'] - h['m_wage'].shift(1))
    ...
    for t in [11,22,24,31,32]:
        plt.clf()
        ax = h[(h['typ_bud']==t) & (h['y_wage'] <= maxinc)].plot.line(x='y_wage',y='emtr')
        fig = ax.get_figure()
        fig.savefig(settings['GRAPH_PATH']+'hypo/emtr_'+str(t)+'.png')
#        for f in [0,1]:
#            plt.clf()
...
#            plt.savefig(graph_path+'hrs_dist/'+str(y)+'_f'+str(f)+'.png')
    ...
    ## WARUM BEKOMMEN LEUTE KEIN ALG2??
    #df['emtr'] = 1 - (df['dpi'].change.shift(1))
    #by typ_bud: gen emtr_`1'       = 1 - ((y_c_`1'[_n] - y_c_`1'[_n-1]) / (y_wage[_n] - y_wage[_n-1]))
